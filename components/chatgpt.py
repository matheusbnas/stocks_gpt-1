...
try:
    sectors_df = pd.read_csv('sectors.csv', index_col=0)
                    ...
                    dbc.CardBody([
                        dbc.Row([
                            dbc.Col([
                                dbc.Row([
                                    dbc.Col([
                                        html.H5([html.I(className='fa fa-desktop', style={"fontSize": '85%'}), " StocksGPT: "], className='textoQuartenario'),
...
layout = dbc.Container([
        dbc.Row([
            dbc.Col([
                dbc.Row([
                    dbc.Col("Stocks Chat", className='textoPrincipal', style={'margin-top' : '10px'}, md=12),
                ], className= 'g-2 my-auto'),
...
def add_msg(n, msg_user):
    ...
    if 'ativo'.lower() or 'ativos'.lower() in msg_user:
        mensagem = f'{df_data_wallet}, considerando todos os dados existentes dentro do dataframe, desde a primeira linha até a última, qual é a resposta exata de um expert para a pergunta: ' + msg_user
    else:
